...
Bonfert-Taylor}}
\author{George Papadopoulos\\ pgeorgios8@gmail.com}
\date{11/4/2017}
...
$\bullet$ (A) $\dsp \{z\in\BBC| |z-3-2\mi|\leq 1\}$\\
$\bullet$ (B) $\dsp \{z\in\BBC | \mathfrak{Im}(z) = 2\}$\\
$\bullet$ (C) $\dsp \left\{z\in\BBC\setminus\{0\}|0<\arg{z}<\frac{\pi}{6}\right\}$\\
$\bullet$ (D) $\dsp \left\{z\in\BBC| |z-1|<|z|\right\}$
\begin{center}
 \textbf{Solution}
\end{center}
In the following figures the \textit{x-axis} is the real axis and the \textit{y-axis} the 
imaginary one.
\begin{center}
\begin{figure}
...
\end{scriptsize}
\end{pspicture*}
\caption{$|z-3-2\mi|\leq 1$ is the unit (r=1) disc (circle+interior) with center (3,2).}
\end{subfigure} \hfill
\begin{subfigure}[t]{0.45\linewidth}
\centering
\psset{xunit=1.0cm,yunit=1.0cm,algebraic=true,dotstyle=o,dotsize=3pt 
...
\caption{It is the colored (light red) region included between the lines 
    $y= \tan\left(\frac{\pi}{6}\right)$ and y=0 without the point (0,0) and of course without 
    the points of the lines mentioned above.} 
\end{subfigure}\hfill
\begin{subfigure}[t]{0.45\linewidth}
\centering
...
\textbf{1.} Find the image of the set $\dsp U = \left\{z\in\mathbb{C} \bigg|  \frac{-\pi}{2}\leq\mathfrak{Re}(z)\leq\frac{\pi}{2}\right\}$ 
under the function $f(z) = \sin(z)$. To do so please answer the following questions:\\
a. What is the image of the line segment $\dsp L_{1} = \left(\frac{\pi}{2},\frac{\pi}{2}\right)$ (in the real axis) under f?\\
b.    What is the image of the imaginary axis $\dsp L_{2}=\left\{\mi y |y \in \mathbb{R}\right\}$ under f?\\
c.    What is the image of the vertical line $L_{3}=\left\{-\frac{\pi}{2}+\mi y\bigg| y\in\mathbb{R}\right\}$ under f?\\
d.    What is the image of the vertical line $\dsp L_{4}=\left\{\frac{\pi}{2}+\mi y \bigg| y\in\mathbb{R}\right\}$ under f?\\
e.    Given your above observations, what do you guess the image of the set U is under f?
\begin{center}
 \textbf{Solution}
\end{center}
...
a. $L_{1}$ is the line segment $\dsp \mathfrak{Re}\left(-\frac{\pi}{2},\frac{\pi}{2}\right)$ in the z-plane, so for 
$\dsp x\in\left(-\frac{\pi}{2},\frac{\pi}{2}\right)$ and y = 0 we get $\dsp w(x,0)=\left<u(x,0),v(x,0)\right>=\left<\sin(x),0\right>$ 
and for $\dsp x = \pm\frac{\pi}{2}$, $\dsp w\left(\pm\frac{\pi}{2},0\right) = \pm 1$, which means that under f, $L_{1}$ is mapped
...
$\mathfrak{Im}(z)$ axis into the imaginary axis $\mathfrak(Im)(w)$ of the \textit{w-plane}.
\[\includegraphics[width = 0.6\linewidth]{1b.png}\]
c. Again for $\dsp x = -\frac{\pi}{2}$ we get $\dsp w\left(-\frac{pi}{2},y\right)=\left<-\cosh(y),0\right>$ and since the range of cosh is 
$[1,+\infty)$ for $y\in\mathbb{R}$, we deduce that f maps the line $\dsp y = -\frac{\pi}{2} + y \mi$ of the \textit{z-plane} into the line 
...
line $\dsp y = -\frac{\pi}{2}+y\mi$ regarding as the center of the infinite verical line the point $\dsp \left(-\frac{\pi}{2},0\right)$ 
leftwards, until both branched become one with the real axis and then the same force moves the new line's edge horizontally to 
...
d. As in (c), $\dsp x = \frac{\pi}{2}$ and so $\dsp w\left(\frac{\pi}{2},y\right)= \left<\cosh(y),0\right>$, so f maps the line 
...
e. It is clear that $\dsp U = \left\{ z \in \BBC\bigg|-\frac{\pi}{2}<\mathfrak{Re}(z)<\frac{\pi}{2}\right\}$ is mapped into the whole
\textit{w-plane} slit along the rays $\mathfrak{Re}(-\infty,1]$ and $\mathfrak{Re}[1,+\infty)$ (white lines in the next figure excluding
also from the w-plane the points (-1,0) and (1,0)).
\[\includegraphics[width = 0.6\linewidth]{1e.png}\]
\textbf{2.} Let $u(x,y) = x^2 - y^2 -y$. Find a real-valued function $v(x,y)$ such that $v(0,0)=1$ and together, u and v satisfy the 
...
$v_y(x,y)$.\\
d.    Find functions v(x,y) that satisfy the equation for the partial derivative with respect to y.\\
e.    Now find a function v(x,y) that satisfies both equations for the partial derivatives at the same time.\\
f.     Finally, check whether the function you found in the previous step satisfies v(0,0)=1. If not, modify the function so that it does.\\
\begin{center}
\textbf{Solution}
\end{center}
$\dsp u(x,y) = x^2 - y^2 -y$\\
...
c. From the previous answer we came up with the equation and by integrating with respect to x and treating y as a 
\textit{\textbf{constant}} we get,\[v_{x}(x,y)=2y+1\Rightarrow \int v_{x}(x,y)\;\mathbb{d}x = \int 2y +1 \;\mathbb{d}x\Rightarrow v(x,y) =
2xy+x+ay+c \;\;(1)\]
for $a,c\in\mathbb{R}$.\\
d. Following the same process as in (c), but now by integrating with respect to y and treating x as a \textit{\textbf{constant}} 
...
\textbf{1.} Sketch the image under the map $f(z)=\textrm{Log}(z)$ of the open half annulus $\dsp A=\left\{z\in\BBC\bigg| 
\mathfrak{e}^{-\frac{\pi}{4}}<|z|<\mathfrak{e}^{\frac{\pi}{4}},\mathfrak{Re}(z)>0\right\}$.\\\\
Recall: $\textrm{Log}(z)$ denotes the principal branch of logarithm, that is, $\textrm{Log}(z)=\ln⁡|z|+\mi \textrm{Arg}(z)$, 
...
coordinate axes, sketch the half annulus A, on a second set of axes sketch its image under f. Please highlight the following 
parts of your graph:\\
1. The set $\dsp \left\{z\in\BBC\bigg||z| =\mathfrak{e}^{-\frac{\pi}{4}}, \mathfrak{Re}(z)>0\right\}$ (this is a part of the boundary
of A) as well as the image of this boundary portion under f in your second graph.\\
2. The set $\dsp \left\{z\in\BBC\bigg||z| = \mathfrak{e}^{\frac{\pi}{4}},\mathfrak{Re}(z)>0\right\}$ (this is another part of the 
boundary of A) as well as the image of this boundary portion under f in your second graph (use a different color for these sets 
than you used in the first part if possible).\\
3. The set $\left\{z \in \BBC \bigg|\mathfrak{e}^{-\frac{\pi}{4}}\leq\mathfrak{Im}(z)\leq\mathfrak{e}^{\frac{\pi}{4}},\mathfrak{Re}(z)=0
\right\}$ (this is yet another part of the boundary of A) as well as the image of this boundary portion under f in your second 
graph (use a third color if possible).\\
4. The set $\dsp \left\{z \in \BBC\bigg|-\mathfrak{e}^{\frac{\pi}{4}}\leq \mathfrak{Im}(z)\leq-\mathfrak{e}^{-\frac{\pi}{4}},
\mathfrak{Re}(z) = 0\right\}$ (this is the fourth part of the boundary of A) as well as the image of this boundary portion under 
...
1. We are given the semicircle $\dsp C_{1}=\left\{z\in\BBC\bigg||z|=\mathfrak{e}^{-\frac{\pi}{4}}, \mathfrak{Re}(z)>0\right\}$ and we 
...
\end{center}
2. We are given the semicircle $\dsp C_{2}=\left\{z\in\BBC\bigg||z|=\mathfrak{e}^{\frac{\pi}{4}}, \mathfrak{Re}(z)>0\right\}$ and 
